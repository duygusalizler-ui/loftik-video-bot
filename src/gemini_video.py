...
SCENE_PROMPT = (
    "The video opens on a completely STILL, static frame: the pair of "
    "shoes worn on a person's feet and lower legs, exactly matching the "
    "reference image (same shoe, same colorway, same design, cropped the "
    "same way as the reference photo). Hold this single still frame "
    "motionless -- no camera movement, no animation, no morphing at all -- "
    "for about the first 1 second of the video. This is the static cover "
    "shot. "
    "Then there is a hard CUT (an abrupt scene cut, NOT a smooth "
    "transition, dissolve, morph, or blend) straight to the next shot: the "
    "exact same pair of shoes, in the exact same colors as the reference "
    "image, now already resting together side by side on top of a round "
    "matte white rotating display podium -- a simple circular turntable "
    "pedestal, like a professional product-photography spinning stand. No "
    "person, foot, leg, ankle or skin appears in this second shot at all -- "
    "the background of this second shot must be a completely empty studio "
    "environment or plain surface with absolutely NO human figure in it, "
    "not even blurred, out-of-focus, or partially visible in the "
    "background. Do not carry any person over from the first shot into the "
    ...
    "background style can vary, that part does not matter). Elegant, "
    "minimal, premium footwear advertisement style, vertical mobile-first "
    "framing. "
    "CRITICAL: do not invent, add, or render ANY brand name, logo, text, "
    "label, or wordmark anywhere on the shoes that is not clearly, "
    "unambiguously present in the reference image -- this includes the "
    "sole, heel, tongue, insole, laces, or any other part. If a part of "
    "the shoe in the reference image is plain/blank, it must stay plain "
    "and blank in the video too -- never fill blank areas with an "
    "invented logo or brand name (for example, do not add words like "
    "'FILA', 'NIKE', or any other real or made-up brand name anywhere). "
    "When in doubt, render that area plain and unbranded rather than "
    "adding any text or logo. "
    "CRITICAL: the graphic PATTERN and DESIGN printed or stitched onto the "
    "shoe's upper -- shapes, letters, panels, stripes, zigzags, or any "
    "distinctive graphic element visible in the reference image -- must be "
    "reproduced EXACTLY as shown, pixel-for-pixel in spirit, in the second "
    "shot. Do NOT redesign, reinterpret, simplify, or replace the shoe's "
    "pattern with a different design (for example, do not turn a "
    "zigzag/letter-shaped panel into diagonal stripes, and do not turn any "
    "design into a style resembling Adidas, Nike, or any other brand's "
    "signature look). The exact same graphic design from the reference "
    "image must still be clearly recognizable on the shoe in every frame "
    "after the cut, unchanged. "
    "CRITICAL: the exact colors of the shoes -- upper material, sole, "
    "laces, every part -- must stay perfectly IDENTICAL between the first "
    "shot and the second shot. Do not lighten, darken, desaturate, or "
    "shift the color of the sole or any other part after the cut. The "
    "sole color in particular must exactly match the reference image's "
    "sole color -- if the sole is white in the reference image, it must "
    "stay exactly that same white in the second shot too; it must never "
    "turn grey or any other shade."
)
...
CLEANUP_PROMPT = (
    ...
    "background with soft studio lighting. No people, no body parts. "
    "Do not add, invent, or render any brand name, logo, or text that is "
    "not clearly present in the original photo -- if a part of the shoe "
    "(sole, heel, tongue, etc.) is plain/blank in the original, keep it "
    "plain and blank; never invent a logo or brand name to fill it in."
)
