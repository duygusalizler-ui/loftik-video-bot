...
SCENE_PROMPT = (
    "This video OPENS exactly as shown in the reference image: the pair of "
    "shoes worn on a person's feet and lower legs, cropped at the ankle/shin "
    "the same way as the reference photo, on plain ground. Hold this exact "
    "opening framing, matching the reference image precisely (same shoe, "
    "same colorway, same design), as a brief still opening shot for about "
    "the first 1 second of the video -- this is the cover shot. "
    "Then, over the next 1-2 seconds, transition smoothly: the legs fade "
    "out of frame and the pair of shoes lifts off, coming to rest together "
    "side by side on top of a round matte white rotating display podium -- "
    "a simple circular turntable pedestal, like a professional "
    "product-photography spinning stand. "
    "From the moment the transition finishes to the end of the video, BOTH "
    "shoes of the pair -- left AND right -- must stay together, side by "
    "side, fully visible at all times. Never drop to showing only one shoe "
    "alone. Only the turntable rotates a full 360 degrees after the "
    "transition, carrying both shoes around together, showing all angles "
    "of the pair, while the camera stays completely FIXED and STATIC for "
    "the entire video -- it does not move, pan, orbit, or zoom at all, not "
    "even during the transition. Soft studio-style lighting, shallow depth "
    "of field, softly blurred background after the transition (the exact "
